...
public class TextProcessing {
    public static void main(String[] args) {
        ...
        // String içerisindeki .,!?\" karakterlerini kaldırıyoruz
        String cleanText = text.replaceAll("[.!?\",]", "");

        // String'i küçük harflere çevirip kelimelere ayırıyoruz
        String[] words = cleanText.toLowerCase().split("\\s+");

        // HashSet ile unique kelimeleri tutuyoruz
        HashSet<String> uniqueWords = new HashSet<>();

        // HashSet'e kelimeleri ekliyoruz
        Collections.addAll(uniqueWords, words);

        // HashSet'i ArrayList'e çevirip sıralıyoruz
        ArrayList<String> sortedUniqueWords = new ArrayList<>(uniqueWords);
        Collections.sort(sortedUniqueWords);

        // Sıralanmış unique kelimeleri yazdırıyoruz
        for (String word : sortedUniqueWords) {
            System.out.println(word);
        }

        // Toplam unique kelime sayısını yazdırıyoruz
        System.out.println("Toplam unique kelime sayisi: " + sortedUniqueWords.size());
    }
}
